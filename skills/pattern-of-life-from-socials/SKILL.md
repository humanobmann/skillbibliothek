...
metadata:
  upstream_disable_model_invocation: true
---
...
This local deployment permits only a bounded analysis of already public,
publication-relevant material after `journalistic-osint-governance` approves
the case. Do not monitor a person's location or routine, profile family or
uninvolved third parties, view stories, follow, like, message, interact, use a
secondary/deceptive account, or turn public posts into a surveillance dossier.
For a public official, report conduct relevant to the story—not private life
merely because the person is prominent. Any remaining sensitive-data or safety
risk is `HOLD FOR REVIEW`.

## Step 1 — Authorized scope

Read [../ETHICS.md](../../references/ETHICS.md), then write down before opening a
single profile:
