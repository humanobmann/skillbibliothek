...
For investigative reporting, `journalistic-osint-governance` must define the
story question and permitted query set first. Search only public indexes and
retain only the minimum snippet, metadata or public record needed to verify a
claim. Do not use a search result as permission to open private content,
retrieve an exposed file, circumvent a platform or build a personal dossier.
Security exposures require a responsible disclosure/CERT and editorial review;
public-interest reporting does not waive privacy, data-protection or platform
terms.
...
gone. Minimise per [../ETHICS.md](../../references/ETHICS.md).
