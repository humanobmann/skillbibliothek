...
For reporting, use this workflow only after `journalistic-osint-governance`
has approved a tightly defined public-interest question. Read already indexed
observations; do not probe, enumerate, retrieve, test, download or authenticate
to a target. A passive scan result is time-bound evidence of an observation,
not permission to access a system or make an unqualified security allegation.
For a third party's estate, require written authority; for a newsworthy exposure,
preserve the minimum necessary detail, use a responsible disclosure or CERT path
where appropriate, and hold publication for editorial/legal review.
...
rules in [../ETHICS.md](../../references/ETHICS.md).
